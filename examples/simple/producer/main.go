...
func onPubFail(msg rabbit.FailedMsg) {
	log.Printf("simple producer failed: msgID=%s body=%s", msg.MessageID, string(msg.Message))
}
...
func main() {
	...
	msgID, err := mq.Publish([]byte("hello simple"))
	if err != nil {
	...
	delayID, err := mq.PublishDelay([]byte("hello simple delay"), delayTTL)
	if err != nil {
