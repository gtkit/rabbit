...
func onPubFail(msg rabbit.FailedMsg) {
	log.Printf("direct producer failed: msgID=%s body=%s", msg.MessageID, string(msg.Message))
}
...
func main() {
	...
	msgID, err := mq.Publish([]byte("hello direct"))
	if err != nil {
	...
	delayID, err := mq.PublishDelay([]byte("hello direct delay"), delayTTL)
	if err != nil {
