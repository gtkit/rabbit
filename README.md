...
`mq` 是一个基于 `github.com/rabbitmq/amqp091-go` 的 RabbitMQ Go 封装，支持以下五种常见消息模型：
...
| `PublishWithDlx(body []byte) (string, error)` | 声明 DLX 拓扑后发布 | Simple ✓ | Direct ✓ | Fanout ✓ | Topic ✓ | Headers ✓ |
| `PublishWithDlxString(msg string) (string, error)` | PublishWithDlx 的字符串便捷包装 | Simple ✓ | Direct ✓ | Fanout ✓ | Topic ✓ |
| `BatchPublish(bodies [][]byte) ([]string, error)` | 批量发布，一次确认 | Simple ✓ | Direct ✓ | Fanout ✓ | Topic ✓ |
