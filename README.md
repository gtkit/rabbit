...
- 可选消费幂等 / 去重：`IdempotentHandler` 装饰器 + `IdempotentStore`，默认不开启（详见四、基础概念）
...
//    分布式多实例需自行实现 IdempotentStore（如基于 Redis），跨进程共享去重状态。
store := rabbit.NewMemoryStore(10 * time.Minute)

// 2. 把业务 handler 包一层，再传给 Consume。
handler := rabbit.IdempotentHandler(myHandler, store)
_ = cons.Consume(handler)
```

去重存储接口（由业务实现并发安全与 TTL）：
...
工作流程：先查 `IsDuplicate` → 命中则跳过业务并 `Ack` → 未命中则执行 `Process` → **仅在处理成功后** `Commit`。

- **与 retry 关系**：`Process` 返回 error 时不 `Commit`，照常走现有 `x-retry` 重试；后续重投会重新处理，保留 at-least-once 语义。
- **去重键**：默认用 `msgID`；当生产者重试导致同一业务消息携带不同 `msgID` 时，用 `WithDedupKey` 改取业务唯一键（返回 `ok=false` 则该消息不参与去重）：

```go
handler := rabbit.IdempotentHandler(myHandler, store,
    rabbit.WithDedupKey(func(body []byte, msgID string) (string, bool) {
...
retry 消息发布成功但原消息 `Ack` 失败时：库记录错误日志、不退出消费循环。仍是 `at-least-once` 语义——可用库提供的 `IdempotentHandler` 装饰器做去重（见「四、5 消费幂等 / 去重」），或在业务侧基于 `msgID` / 业务唯一键自行幂等。
...
4. **消费去重 / 幂等**：可用库提供的 `IdempotentHandler` 装饰器（见「四、5」），或业务侧基于 `msgID` / 业务唯一键自行幂等
